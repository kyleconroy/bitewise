...
if not options.consumer_key:
  if os.environ.has_key("YELP_CONSUMER_KEY"):
    options.consumer_key = os.environ.get("YELP_CONSUMER_KEY")
  else:
    parser.error('--consumer_key required')
if not options.consumer_secret:
  if os.environ.has_key("YELP_CONSUMER_SECRET"):
    options.consumer_secret = os.environ.get("YELP_CONSUMER_SECRET")
  else:
    parser.error('--consumer_secret required')
if not options.token:
  if os.environ.has_key("YELP_TOKEN"):
    options.token = os.environ.get("YELP_TOKEN")
  else:
    parser.error('--token required')
if not options.token_secret:
  if os.environ.has_key("YELP_TOKEN_SECRET"):
    options.token_secret = os.environ.get("YELP_TOKEN_SECRET")
  else:
...
while True:
  ...
  filename = "data/{0}/{0}_offest_{1:06}.json".format(options.category.lower(),
                                                      offset)
